...
export async function validationMiddleware(
  ...
  try {
    ...
    if (ctx.message?.text && ctx.message.text.length > 4096) {
      await ctx.reply("❌ Message is too long. Maximum length is 4096 characters.");
      return;
    }

    // Validate caption length for media
    if (ctx.message?.caption && ctx.message.caption.length > 1024) {
      await ctx.reply("❌ Caption is too long. Maximum length is 1024 characters.");
      return;
    }
...
async function validateMediaUpload(ctx: BotContext): Promise<MediaValidationResult> {
  ...
  if (ctx.message?.video) {
    ...
    if (video.duration && video.duration > 600) { // 10 minutes max
      return {
        valid: false,
  ...
  if (data.buttons) {
    ...
    for (const [index, button] of data.buttons.entries()) {
      ...
      if (button.callbackData && button.callbackData.length > 64) {
        errors.push(`Button ${index + 1}: Callback data too long (max 64 characters)`);
      }
    }
  }
