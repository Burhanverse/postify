  ...
  switch (errorCode) {
    case 400:
      ...
      if (description.includes("chat not found")) {
        await safeReply(ctx, "❌ Channel not found. Please check your channel settings with /checkchannels");
        return;
      }
      break;
    ...
    case 403:
      ...
      if (description.includes("not enough rights")) {
        await safeReply(ctx, "❌ Bot doesn't have enough permissions. Please grant admin rights to the bot.");
        return;
      }
      break;
    
    case 429:
      await safeReply(ctx, "⏳ Too many requests. Please wait a moment and try again.");
      return;
    
    case 500:
    case 502:
    case 503:
      await safeReply(ctx, "🔧 Telegram is experiencing issues. Please try again in a few minutes.");
      return;
  }
  
  // Generic Grammy error
  await safeReply(ctx, "❌ An error occurred while processing your request. Please try again.");
}
...
async function handleHttpError(ctx: BotContext, error: HttpError) {
  ...
  await safeReply(ctx, "🌐 Network error. Please check your connection and try again.");
}
