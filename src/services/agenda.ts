  ...
  agenda.define("publish_post", { 
    ...
    try {
      ...
      if (post.status !== 'scheduled') {
        jobLogger.warn(`Post ${postId} is not scheduled (status: ${post.status}), skipping`);
        return;
      }
      ...
      if (post.channel) {
        ...
        if (userId && !channel.owners.includes(userId)) {
          ...
          throw new Error(`User ${userId} no longer has access to channel for post ${postId}`);
        }
      }
      ...
      try {
        ...
        jobLogger.error({ error: updateError }, "Failed to update post status after error");
      }
...
export async function schedulePost(
  ...
  logger.warn("Using deprecated schedulePost function. Consider using PostScheduler service instead.");
...
export async function getSchedulingStats() {
  try {
    const stats = {
      totalScheduled: (await agenda.jobs({ 
        name: "publish_post", 
        nextRunAt: { $ne: null },
        disabled: { $ne: true }
      })).length,
      
      failedJobs: (await agenda.jobs({ 
        name: "publish_post", 
        failedAt: { $ne: null },
        nextRunAt: null
      })).length,
      
      completedToday: (await agenda.jobs({
        name: "publish_post",
        lastFinishedAt: { 
