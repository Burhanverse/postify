...
export class PostScheduler {
  ...
  public parseScheduleInput(
    ...
    try {
      ...
      const relativeMatch = trimmed.match(
        /^in\s+(\d+)\s*(m|min|mins|minutes|h|hr|hrs|hours|d|day|days)?\s*$/i,
      );
      if (relativeMatch) {
        ...
        if (unit.startsWith("m")) {
          if (amount < 1 || amount > 10080) {
            // 1 week in minutes
            return {
              valid: false,
              error: "Minutes must be between 1 and 10,080 (1 week)",
            };
          }
          ...
          if (amount < 1 || amount > PostScheduler.MAX_SCHEDULE_DAYS) {
            return {
              valid: false,
              error: `Days must be between 1 and ${PostScheduler.MAX_SCHEDULE_DAYS}`,
            };
          }
          ...
          return {
            valid: false,
            error: "Invalid time unit. Use 'm', 'h', or 'd'",
          };
        }
      ...
      const naturalLanguageResult = this.parseNaturalLanguage(
        trimmed,
        timezone,
      );
      ...
      // Try common date formats
        ...
        parsedDate = DateTime.fromFormat(input, "yyyy-MM-dd HH:mm:ss", {
          zone: timezone,
        });
        if (!parsedDate.isValid) {
          parsedDate = DateTime.fromFormat(input, "yyyy-MM-dd HH:mm", {
            zone: timezone,
          });
        }
      }
      // Try short format
        ...
        parsedDate = DateTime.fromFormat(input, "MM/dd/yyyy HH:mm", {
          zone: timezone,
        });
      }
      // Try just time (today)
      ...
      if (parsedDate.toUTC() > maxDate) {
        return {
          valid: false,
          error: `Cannot schedule more than ${PostScheduler.MAX_SCHEDULE_DAYS} days in advance`,
        };
      }
      ...
      logger.error(
        ...
        "Error parsing schedule input",
      );
      return {
        valid: false,
        error: "Failed to parse date/time. Please check your format.",
      };
    }
  }
  ...
  private parseNaturalLanguage(
    ...
    try {
      ...
      const tomorrowMatch = input.match(
        /^tomorrow(?:\s+(\d{1,2}):(\d{2})(?::(\d{2}))?)?$/,
      );
      if (tomorrowMatch) {
        ...
        if (tomorrowMatch[1] && tomorrowMatch[2]) {
          ...
          if (
            ...
            return {
              valid: false,
              error:
                "Invalid time format. Use HH:MM or HH:MM:SS (24-hour format)",
            };
          }
      ...
      const dayMatch = input.match(
        /^(?:(next|this)\s+)?(monday|tuesday|wednesday|thursday|friday|saturday|sunday)(?:\s+(\d{1,2}):(\d{2})(?::(\d{2}))?)?$/,
      );
      if (dayMatch) {
      ...
      const todayMatch = input.match(
        /^today\s+(\d{1,2}):(\d{2})(?::(\d{2}))?$/,
      );
      if (todayMatch) {
        ...
        if (today <= now) {
          return {
            valid: false,
            error:
              "That time has already passed today. Use 'tomorrow' or a future time.",
          };
        }
  ...
  public async checkSchedulingConflicts(
    ...
    try {
      ...
      const intervalStart = scheduledAt
        .minus({ minutes: PostScheduler.MIN_POST_INTERVAL })
        .toJSDate();
      const intervalEnd = scheduledAt
        .plus({ minutes: PostScheduler.MIN_POST_INTERVAL })
        .toJSDate();
  ...
  public async schedulePost(request: ScheduleRequest): Promise<{
    ...
    try {
      ...
      if (post.authorTgId !== userId) {
        return {
          success: false,
          error: "Unauthorized: Post does not belong to user",
        };
      }
      ...
      if (!channel.owners.includes(userId)) {
        return {
          success: false,
          error: "Unauthorized: User does not have access to channel",
        };
      }
